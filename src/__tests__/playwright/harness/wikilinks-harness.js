...
`);
  ...
  var noteIdentifiers = /* @__PURE__ */ new Set();
  var noteTitleMap = /* @__PURE__ */ new Map();
  var noteIndexChangeCallback = null;
  function setWikilinkNoteIndex(identifiers) {
  ...
  var WikilinkNode = Node3.create({
    ...
    addNodeView() {
      return ({ node, getPos: _getPos }) => {
        ...
        dom.addEventListener("mouseenter", () => {
          const win = window;
          const dismissTimer = win.__wikilinkDismissTimer;
          if (dismissTimer) {
    ...
    addCommands() {
      return {
        insertWikilink: (identifier) => ({ commands }) => {
  ...
  function Suggestion({
    ...
    const getAnchorClientRect = () => {
      const pos = editor2.state.selection.$anchor.pos;
    ...
    const clientRectFor = (view, decorationNode) => {
      ...
      return () => {
        const state = pluginKey.getState(editor2.state);
      ...
      try {
        ...
        const exitProps = {
          ...
          command: (commandProps) => {
            return command2({ editor: editor2, range: (state == null ? void 0 : state.range) || { from: 0, to: 0 }, props: commandProps });
          },
          decorationNode,
      ...
      view() {
        return {
          update: async (view, prevState) => {
            ...
            if (handleExit) {
              (_e2 = renderer == null ? void 0 : renderer.onExit) == null ? void 0 : _e2.call(renderer, props);
            }
            if (handleChange) {
  ...
  var index_default3 = Suggestion;

  // src/webview/extensions/WikilinkSuggestion.ts
  var noteCache = [];
  function setWikilinkSuggestionNotes(notes) {
    noteCache = notes;
  }
  function filterNotes(query) {
    const lowerQuery = query.toLowerCase().trim();
    if (!lowerQuery) return noteCache.slice(0, 15);
    return noteCache.filter(
      (note) => note.title.toLowerCase().includes(lowerQuery) || note.identifier.toLowerCase().includes(lowerQuery) || note.aliases.some((a) => a.toLowerCase().includes(lowerQuery))
    ).slice(0, 15);
  }
  function buildItems(query) {
    const matches2 = filterNotes(query);
    if (matches2.length === 0 && query.trim()) {
      return [
        {
  ...
  function renderDropdown(el, items, selectedIndex, query, onSelect, onHover) {
    el.innerHTML = "";
    ...
    if (items.length === 0) {
      ...
      empty2.className = "wikilink-suggestion__empty";
      empty2.textContent = query.trim() ? `No notes matching "${query}"` : "No notes in index";
      el.appendChild(empty2);
    }
  }
  function positionDropdown(el, decorationNode) {
    const anchor = decorationNode ?? window.getSelection()?.getRangeAt(0)?.getBoundingClientRect();
    if (!anchor) return;
    const rect = anchor instanceof Element ? anchor.getBoundingClientRect() : anchor;
    el.style.left = `${rect.left}px`;
    el.style.top = `${rect.bottom + 4}px`;
  }
  var WikilinkSuggestion = Extension.create({
    ...
    addProseMirrorPlugins() {
      return [
        index_default3({
          ...
          command: ({ editor: editor2, range, props: note }) => {
  ...
  function handleWikilinkPreview(identifier, excerpt, broken) {
    ...
    document.getElementById("wikilink-preview-tooltip")?.remove();
    const rect = win.__wikilinkHoverRect;
    if (!rect) return;
    const titleMap = win.__wikilinkTitleMap;
  ...
  function seedNoteIndex(notes) {
    ...
    window.__wikilinkTitleMap = titleMap;
  }
...
//# sourceMappingURL=wikilinks-harness.js.map
