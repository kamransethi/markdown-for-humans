...
describe('pasteHandler', () => {
  describe('htmlToMarkdown', () => {
    ...
    it('should convert standard HTML task list items', () => {
      ...
      const result = htmlToMarkdown(html);
      expect(result).toMatch(/- \s*\[x\] \s*Task done/);
      expect(result).toMatch(/- \s*\[ \] \s*Task todo/);
    });
...
<ul data-type="taskList">
  ...
  <li data-type="taskItem" data-checked="false">
    ...
    <div><p>Document decisions</p></div>
      ...
      const result = htmlToMarkdown(html);
      expect(result).toMatch(/- \s*\[x\] \s*Plan file created/);
      expect(result).toMatch(/- \s*\[ \] \s*Document decisions/);
...
<ul data-type="taskList">
  ...
  describe('processPasteContent', () => {
    ...
    it('should convert external HTML with checkboxes to markdown (not HTML)', () => {
      ...
      expect(result.isMarkdown).toBe(true);
      expect(result.isHtml).toBe(false);
      expect(result.content).toMatch(/- \s*\[x\] \s*Buy milk/);
      expect(result.content).toMatch(/- \s*\[ \] \s*Write code/);
    });
    ...
    it('should convert TipTap task list HTML to markdown (not HTML)', () => {
      ...
      expect(result.isMarkdown).toBe(true);
      expect(result.isHtml).toBe(false);
      expect(result.content).toMatch(/- \s*\[x\] \s*Done task/);
      expect(result.content).toMatch(/- \s*\[ \] \s*Todo task/);
