...
export class FoamGraph extends LitElement {
  ...
  updated(changed: Map<string, unknown>) {
    ...
    if (changed.has('graphData')) {
      this.graphModel = this.graphData ? createGraphModel(this.graphData) : null;
    ...
    if ((changed.has('graphData') || changed.has('graphStyle')) && this.graphModel) {
  ...
  render() {
    ...
    const previewMatchCount = this.graphModel
      ? computePreviewMatchCount(this.graphModel, this.groupDraft.property, this.groupDraft.value)
      : 0;
    ...
    return html`
      <foam-graph-canvas
        ...
        @canvas-node-click=${(e: CustomEvent) => this._onCanvasNodeClick(e.detail)}
        @canvas-node-hover=${(e: CustomEvent) => (this.hoverNodeId = e.detail)}
        @canvas-background-click=${(e: CustomEvent) => this._onCanvasBackgroundClick(e.detail)}
      ></foam-graph-canvas>
      ${this.showControls
        ? html`<foam-control-panel
            ...
            @toggle-node-type=${(e: CustomEvent) => this._onToggleNodeType(e.detail)}
            ...
            @cancel-draft=${() => (this.groupDraft = { ...this.groupDraft, active: false })}
  ...
  selectNote(noteId: string) {
    ...
    if (this.selection.centerOnSelect) {
      this.canvas?.centerOnNode(noteId, this.selection.zoomOnSelect ? 3 : undefined, 300);
  ...
  private _pruneInteractionState() {
    ...
    this.selectedNodeIds = new Set(
      [...this.selectedNodeIds].filter(id => this.graphModel!.nodeInfo[id] != null)
    );
