      ...
      <details open>
        ...
        <div class="section-content">
          <label class="select-row">
            <span>Color by</span>
            <select
              .value=${this.graphStyle.colorMode ?? 'none'}
              @change=${(e: Event) =>
                this._emit('style-change', {
                  colorMode: (e.target as HTMLSelectElement).value as 'none' | 'directory' | 'type',
                })}
            >
          ...
          ${this._specialTypes.map(
            type => html`
              <div class="checkbox-row">
                ...
                />
                <span style="flex:1"
                  >${ControlPanel._SPECIAL_LABELS[type]} (${this.nodeTypeCounts[type] ?? 0})</span
                >
                ${this.graphStyle.colorMode !== 'directory' ||
                type === 'tag' ||
                type === 'placeholder'
                  ? html`<span
                      class="group-dot"
                    ...
                    >
                      <span style="color: ${getTypeColor(type, this.graphStyle)}">●</span>
                      <input
                        type="color"
                        .value=${getTypeColor(type, this.graphStyle)}
                        @input=${(e: Event) =>
                          this._emit('style-change', {
                            node: {
                              ...this.graphStyle.node,
          ...
          ${this.groups.map(
            (group, i) => html`
              <div class="checkbox-row">
                ...
                <button class="icon-btn" title="Delete group" @click=${() => this._deleteGroup(i)}>
                ...
                <div class="add-group-form">
                  <div class="add-group-form-row">
                    <select
                      ...
                      @change=${(e: Event) =>
                        this._emitDraftChange({
                          property: (e.target as HTMLSelectElement).value as GroupMatch['property'],
                        })}
                    >
                      <option value="type">type</option>
                    ...
                    <foam-autocomplete-input
                      ...
                      placeholder=${ControlPanel._MATCH_HINT[this.groupDraft.property] ?? 'exact'}
                  ...
                  <div class="add-group-form-actions">
                    ${this.groupDraft.value
                      ? html`<span style="opacity:0.6;font-size:11px;flex:1"
                          >${this.previewMatchCount} note${this.previewMatchCount === 1 ? '' : 's'}
                          match</span
                        >`
                    ...
                    <button class="btn" @click=${() => this._emit('add-group', null)}>Add</button>
                  </div>
                </div>
              `
            : html`
                <button class="add-group-btn" @click=${() => this._emit('start-draft', null)}>
                  + Add group
                </button>
              `}
        </div>
        ...
        <div class="section-content">
          <label class="slider-row">
            ...
            <input
              ...
              @input=${(e: Event) =>
                this._emit('text-fade-change', parseFloat((e.target as HTMLInputElement).value))}
            />
          ...
          <label class="select-row">
            ...
            <select
              ...
              @change=${(e: Event) =>
                this._emit('animate-links-change', (e.target as HTMLSelectElement).value)}
            >
        ...
        <div class="section-content">
          ...
          <label class="slider-row">
            ...
            <input
              ...
              @input=${(e: Event) =>
                this._emitForcesChange({
                  velocityDecay: parseFloat((e.target as HTMLInputElement).value),
                })}
            />
