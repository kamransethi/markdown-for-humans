 ...
 */
export function mergeStyles(base: GraphStyle | null, patch: GraphStyle): GraphStyle {
  return {
    ...base,
    ...patch,
 ...
 */
export function resolveStyle(payload: GraphStyle | null, defaults: ResolvedStyle): ResolvedStyle {
  if (!payload) return defaults;
  return {
    ...defaults,
    ...payload.style,
    lineColor: payload.style?.lineColor || payload.style?.node?.note || defaults.lineColor,
    node: {
      ...defaults.node,
      ...payload.style?.node,
    },
