...
import type { GraphModel, GraphModelNode, GraphScope, ResolvedStyle } from './types';
...
export function computeNodeTypeCounts(graphModel: GraphModel | null): Record<string, number> {
...
export function computeAutocompleteOptions(
  ...
  if (property === 'tag') {
    return [...new Set(nodes.flatMap(node => (node.tags ?? []).map(tag => tag.label)))].sort();
  }
  return [];
}
...
export function computePreviewMatchCount(
  ...
  return Object.values(graphModel.nodeInfo).filter(node => matchesGroup(node, rule)).length;
}
